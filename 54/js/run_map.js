...
var settings = {"marginRight": "0", 
        "color_domain": [0, 300, 900, 10000, 100000], 
        "legend_labels": ["0", "300+", "900+" , "100 000+", "150 000+", "190 000"],
        "labels_rect_color": [ "#fff", "#fff", "#ffffb2", "#fecc5c", "#fd8d3c", "#f03b20", "#bd0026" ],
        "zoom" : true,
        "map_json": "./map_assets/russia_mercator.json",
        "district": "./map_assets/district.json",
        "population": "./map_assets/population.csv",
                "population": "./data (3).csv",
...
(function($) {
  ...
  nameById = {},
  period = "2013";
  period = $('#select-year option:selected').val();
  ...
  _Global_district = district;
    data.forEach(function(d, i) {
      ...
      countById[d.Code] = {"2013im": d['year2013im'], 
                 "2014im": d['year2014im'],
                 "2015im": d['year2015im'],
                 "2016im": d['year2016im'],
                 "2013ex": d['year2013ex'],
  ...
  function getTextTooltip (d) {
    if(typeof settings.viewDistrict != "undefined" && settings.viewDistrict) {
      ...
      p = period + ext
      return nameById[d.properties.ISO_2] + " : " + new Number(countById[d.properties.ISO_2][p]).gracefulFormatNumber(0, " ");
    }
  }
  ...
  function getColor (d) {
    if(typeof settings.viewDistrict != "undefined" && settings.viewDistrict) {
      ...
      reg = countById[d.properties.ISO_2]
      if(settings.viewExport) {
        ext = 'ex'
      }
      else {
  ...
  function selectMode() {
    settings.viewExport = (this.value === 'true');
